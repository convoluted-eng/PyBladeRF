"""
MIMO TX/RX Implementation for BladeRF SDR
==========================================

This script implements a MIMO (Multiple Input Multiple Output) transmit and receive
system using the BladeRF Software Defined Radio platform. It demonstrates:

- Dual-channel (2x2 MIMO) transmission and reception
...
Features:
- Transmits a 500 kHz tone at 1 GHz center frequency
- Concurrent measurement and reference channel processing
- Bandpass filtering for received signals
- Comprehensive signal analysis and plotting

Hardware Requirements:
- BladeRF SDR device
- Appropriate RF connections for MIMO operation
...
def receive(sdr, rx_meas_ch, num_samples, tx_start_event, rx_done_event):
    """
    Handle the reception and processing of IQ samples on measurement and reference channels.
    ...
    bytes_per_sample = 4
    buf = bytearray(2 * 1024 * bytes_per_sample)  # Buffer for both channels
    
    # Design bandpass filter for received signal (centered at 500 kHz)
    nyquist = sample_rate / 2
    low = (500e3 - 50e3/2) / nyquist
    high = (500e3 + 50e3/2) / nyquist
    ...
    meas_buf = np.zeros(num_samples, dtype=np.complex64)
    ref_buf = np.zeros(num_samples, dtype=np.complex64)
    num_samples_read = 0
    condition = False
    ...
    while condition == False:
        if repeats_remaining < 30:
            ...
            while num_samples_read < num_samples:
                ...
                # Read IQ data from both channels
                ...
                # Deinterleave MIMO channels (Ch0 I/Q, Ch1 I/Q pattern)
                ...
                meas_buf[num_samples_read:num_samples_read+num] = meas_samples_scaled[0:num]
    ...
    print("Is meas_buf complex =", np.iscomplexobj(meas_buf))
    print("First 20 values of meas_buf =", meas_buf[0:20])
    ...
    meas_unfiltered = meas_buf[int(18.5e3):].copy()
    ref_unfiltered = ref_buf[int(18.5e3):].copy()
...
rx_ref_ch = sdr.Channel(_bladerf.CHANNEL_RX(1))   # RX reference channel
tx_meas_ch = sdr.Channel(_bladerf.CHANNEL_TX(0))  # TX measurement channel
tx_ref_ch = sdr.Channel(_bladerf.CHANNEL_TX(1))   # TX reference channel

# Configuration parameters
sample_rate = 10e6    # 10 MSPS
bandwidth = 500e3     # 500 kHz bandwidth
center_freq = 1e9     # 1 GHz center frequency
rx_gain = 60          # RX gain: -15 to 60 dB
tx_gain = 56          # TX gain: -15 to 60 dB
num_samples = int(20e3)  # 20k samples per transmission
...
# Configure RX reference channel
rx_ref_ch.frequency = center_freq
...
# Configure TX reference channel
tx_ref_ch.frequency = center_freq
tx_ref_ch.sample_rate = sample_rate
tx_ref_ch.bandwidth = bandwidth
tx_ref_ch.gain = tx_gain
...
print("RX ref frequency:", sdr.get_frequency(_bladerf.CHANNEL_RX(1)))
...
print("TX ref frequency:", sdr.get_frequency(_bladerf.CHANNEL_TX(1)))
...
# Generate time vector and test tone (500 kHz complex exponential)
...
# Create interleaved MIMO buffer (Ch0 I/Q, Ch1 I/Q pattern)
samples_tx = np.empty(2 * len(tone), dtype=np.int16)
samples_tx[0::2] = np.real(tone_scaled).astype(np.int16)  # I samples
samples_tx[1::2] = np.imag(tone_scaled).astype(np.int16)  # Q samples
...
# Configure synchronous TX stream (2-channel MIMO)
...
# Configure synchronous RX stream (2-channel MIMO)
...
time_vector = np.arange(len(meas_samples)) / sample_rate

# Analysis parameters for time domain plots
start_time_ms = 59.98  # Start time in milliseconds
end_time_ms = 60.11    # End time in milliseconds
...
plt.subplot(5, 1, 2)
plt.plot(time_vector * 1000, np.real(meas_samples), 'b-', label='I (Real)')
plt.plot(time_vector * 1000, np.imag(meas_samples), 'r-', label='Q (Imag)')
plt.xlabel("Time [ms]")
...
# 3. Reference channel average spectrum
plt.subplot(5, 1, 3)
avg_spectrum = np.zeros(fft_size)

for i in range(num_segments):
    segment = ref_samples[i*fft_size:(i+1)*fft_size]
    spectrum = np.abs(np.fft.fftshift(np.fft.fft(segment)))**2
    avg_spectrum += spectrum
...
# 4. Reference channel time domain (I/Q components)
plt.subplot(5, 1, 4)
plt.plot(time_vector * 1000, np.real(ref_samples), 'b-', label='I (Real)')
plt.plot(time_vector * 1000, np.imag(ref_samples), 'r-', label='Q (Imag)')
plt.xlabel("Time [ms]")
plt.ylabel("Amplitude")
plt.title("Reference Channel Time Domain (I/Q)")
plt.legend()
plt.grid(True)

# 5. Original transmitted tone for comparison
plt.subplot(5, 1, 5)
...
# Optional phase analysis plots (currently commented out)
"""
plt.subplot(4, 1, 1)
plt.plot(time_vector * 1000, np.angle(meas_samples))
plt.xlabel("Time [ms]")
plt.xlim(start_time_ms, end_time_ms)
plt.ylabel("Phase")
plt.title("Measurement Channel Time Domain Phase")
plt.grid(True)

plt.subplot(4, 1, 2)
plt.plot(time_vector * 1000, np.angle(ref_samples))
plt.xlabel("Time [ms]")
plt.xlim(start_time_ms, end_time_ms)
plt.ylabel("Phase")
plt.title("Reference Channel Time Domain Phase")
plt.grid(True)

plt.subplot(4, 1, 3)
plt.plot(t[int(18.5e3):] * 1000, np.angle(tone[int(18.5e3):]))
plt.xlabel("Time [ms]")
plt.xlim(start_time_ms, end_time_ms)
plt.ylabel("Phase")
plt.title("Tone Time Domain Phase")
plt.grid(True)

plt.subplot(4, 1, 4)
plt.plot(time_vector * 1000, np.angle(compensated))
plt.xlabel("Time [ms]")
plt.xlim(start_time_ms, end_time_ms)
plt.ylabel("Phase")
plt.title("Compensated Signal Time Domain Phase")
plt.grid(True)
"""
